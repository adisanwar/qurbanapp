...
class UserController extends GetxController {
  ...
  Stream<UserModel> streamUserDetails(String docId) {
  final controller = StreamController<UserModel>();
  // Fetch user details once
  FirebaseFirestore.instance.collection('users').doc(docId).snapshots().listen((userSnapshot) {
    if (userSnapshot.exists) {
      final user = UserModel.fromSnapshot(userSnapshot);
      controller.add(user); // Emit the user data
    } else {
      controller.addError('User not found'); // Emit an error if user is not found
    }
  });
